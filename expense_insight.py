...
def _(df, mticker, pl, plt):
    ...
    for i, month in enumerate(last3):
        ax = ax_m[i]
        month_df = monthly_by_cat.filter(pl.col("month") == month).sort(
    ...
    month0_df = (
        monthly_by_cat.filter(pl.col("month") == last3[0]).sort(
            "total", descending=True
        )
        if last3
        else None
    ...
    plt.savefig("expense_analysis.png", dpi=150, facecolor=BG)
    plt.show()
    return
